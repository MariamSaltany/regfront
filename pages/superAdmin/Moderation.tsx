...
// ✅ Local helper type to allow user on Review
...
const AdminModeration: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPending = async () => {
      try {
        // ✅ Fetch URL should be this:
        const res = await api.get('/admin/moderation');

        // ✅ FIXED response shape (paginate object)
        ...
        setReviews([
          {
            ...
            user: { name: 'John Smith', email: 'john@smith.com' },
              ...
              <div className="bg-neutral-50 px-6 py-4 border-b flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm font-bold text-secondary">
                    {r.user?.name || 'Parent'} ({r.user?.email || '—'})
                  </p>
                </div>
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm font-bold text-textDark">{(r as any).school?.name}</p>
                </div>
              </div>
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <div className="flex items-center gap-4 mb-4">
                    <div className="bg-secondary/10 p-2 rounded text-secondary font-bold text-sm">
                      ID: {(r as any).student_number}
                    </div>
                    <div className="text-xs text-textLight">
                      {new Date((r as any).created_at).toLocaleString()}
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    {[
                      { label: 'Hygiene', val: (r as any).hygiene },
                      { label: 'Mgmt', val: (r as any).management },
                      { label: 'Quality', val: (r as any).education_quality },
                      { label: 'Comm', val: (r as any).parent_communication },
                    ].map((s) => (
                      <div
                        key={s.label}
                        className="bg-neutral-50 p-2 rounded border border-neutral-100"
                      >
                ...
                <div className="flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-sm text-textDark italic">
                      "{(r as any).comment || 'No comment provided'}"
                    </p>
                  </div>
          ...
          <div className="bg-white w-full max-w-md p-6 rounded-lg shadow-xl">
            ...
            <div className="flex gap-3 justify-end">
              <button
                ...
                className="px-4 py-2 text-textLight hover:bg-neutral-100 rounded"
              >
                Cancel
              </button>
              <button
                onClick={handleReject}
                className="px-6 py-2 bg-warning text-white rounded font-bold hover:bg-red-600"
              >
                Confirm Reject
              </button>
            </div>
