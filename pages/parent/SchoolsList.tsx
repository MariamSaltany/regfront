    ...
    <div>
      <div className="mb-10 text-center">
        ...
        <p className="text-textLight max-w-2xl mx-auto">
          Discover top-rated educational institutions, read verified parent
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSchools.map((school) => (
            <div
              key={school.id}
              className="bg-white rounded-lg shadow-sm border border-neutral-100 overflow-hidden hover:shadow-md transition group"
            >
              <div className="h-48 bg-neutral-100 flex items-center justify-center p-4">
                {school.logo ? (
                  <img
                    src={`${school.logo}?t=${Date.now()}`}
                    alt={school.name}
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-xl font-bold text-textDark group-hover:text-secondary transition">
                    {school.name}
                  </h3>
